...
LveRenderer::LveRenderer(LveWindow& window, LveDevice& device)
    ...
    vkCreateDescriptorPool(device.device(), &pool_info, VK_NULL_HANDLE, &imGuiDescriptorPool);
    ...
    if (vkCreateDescriptorSetLayout(device.device(), &layoutInfo, nullptr, &samplerSetLayout) != VK_SUCCESS) {
