...
const uint32_t TextureLibrary::INVALID_TEXTURE_INDEX = 999;

TextureLibrary::TextureLibrary(LveDevice& device, LveRenderer& renderer)
	: lveDevice(device)
	, renderer(renderer)
...
uint32_t TextureLibrary::AddTexture(const std::string& texturePath) {
	...
	if (textureMap.find(texturePath) == textureMap.end()) {
		...
		textures.push_back(std::make_unique<Texture>(lveDevice, texturePath));
...
uint32_t TextureLibrary::AddTextureFromMemory(const unsigned char* textureData, int textureSize)
{
	...
	textures.push_back(std::make_unique<Texture>(lveDevice, newPath, textureData, textureSize));
